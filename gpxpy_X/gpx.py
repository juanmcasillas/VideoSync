...
# jmc hack support garmin extensions
...
class GPXTrackPoint(mod_geo.Location):
    ...
    def __repr__(self):
        ...
        for attribute in 'elevation', 'time', 'symbol', 'comment', 'horizontal_dilution', \
                'vertical_dilution', 'position_dilution', 'speed', 'name', 'extensions':
            value = getattr(self, attribute)
            if value is not None:
                representation += ', %s=%s' % (attribute, repr(value))
...
class GPXTrack:
    ...
    def get_center(self):
        ...
        center = geo.Location ( bounds.min_latitude +  ((bounds.max_latitude - bounds.min_latitude) / 2),
    ...
    def HasExtensions(self,extname=None):
        # check if some of the points have the extension attribute,
        # if extname is !None, check if it its in the keys
        #
        if not self.segments:
            return False

        p = self.segments[0].points[0]
        if p.extensions != None and extname == None:
            return True


        if extname != None and p.extensions != None and extname in p.extensions.keys() and p.extensions[extname] != None:
            return True
    ...
    def get_average_altitude(self):
        """
        Compute the average altitude (Sum(altitudes)/#Points)
    ...
    def get_average_speed(self):
        ...
        mt, st, md, sd, maxs = self.get_moving_data()
    ...
    # jmc hack - get the distance using the ANT+ data
    def antplus_distance(self):
        if self.HasExtensions("distance"):
            return self.segments[0].points[-1].extensions['distance'] #meters, last record.
        else:
            return 0
...
class GPXTrackSegment:
    ...
    def get_uphill_downhill(self):
        ...
        # jmc hacks. 0 elevations are bad elevations. skip it

        elevations = filter(lambda x: x> 0, elevations)
    ...
    def get_elevation_extremes(self):
		...
		# jmc hack. Elevation (min) is always > 0 (0 is the min xD)
		# can't go underwater for now :P

        elevations = map(lambda location: location.elevation, self.points)
        elevations = filter(lambda elevation: elevation is not None and elevation > 0, elevations)
