...
class GPXParser:
    ...
    def __parse_dom(self):
        ...
        # jmc hack to get gpx version
        if self.xml_parser.get_node_attribute(node, "version"):
            self.gpx.version = self.xml_parser.get_node_attribute(node, "version")
    ...
    def __parse_track_point(self, node):
        ...
        #
        # jmc hack to get GPX extensions (note the V2 ...)
        #
        ...
        if extensions_node:
            ...
            ex_node =  self.xml_parser.get_first_child(extensions_node, 'gpxtpx:TrackPointExtension')

            # v2
            if ex_node:

                #
                # if gpx extensions are available...
                # get hr, cad, speed data
                #
                extensions = { 'hr': None, 'cad': None, 'speed': None, 'power': None, 'left_pedal_smoothness': None, 'left_torque_effectiveness': None, 'distance': None, 'temperature': None }

                cur_node =  self.xml_parser.get_first_child(ex_node, 'gpxtpx:hr')
                if cur_node:
                    ex = mod_utils.to_number(self.xml_parser.get_node_data(cur_node))
                    extensions['hr'] = int(ex)

                cur_node =  self.xml_parser.get_first_child(ex_node, 'gpxtpx:cad')
                if cur_node:
                    ex = mod_utils.to_number(self.xml_parser.get_node_data(cur_node))
                    extensions['cadence'] = int(ex)
                    extensions['cad'] = int(ex)

                cur_node =  self.xml_parser.get_first_child(ex_node, 'gpxtpx:speed')
                if cur_node:
                    ex = mod_utils.to_number(self.xml_parser.get_node_data(cur_node))
                    extensions['speed'] = int(ex)

                cur_node =  self.xml_parser.get_first_child(ex_node, 'gpxtpx:distance')
                if cur_node:
                    ex = mod_utils.to_number(self.xml_parser.get_node_data(cur_node))
                    extensions['distance'] = int(ex)

                cur_node =  self.xml_parser.get_first_child(ex_node, 'gpxtpx:atemp')
                if cur_node:
                    ex = mod_utils.to_number(self.xml_parser.get_node_data(cur_node))
                    extensions['temperature'] = int(ex)
                    extensions['atemp'] = int(ex)

            # v3
            # watch out the namespace .... :/
            if not ex_node:
                ex_node =  self.xml_parser.get_first_child(extensions_node, 'ns3:TrackPointExtension')
                if ex_node:
                    extensions = { 'hr': None, 'cad': None, 'speed': None, 'power': None, 'left_pedal_smoothness': None, 'left_torque_effectiveness': None, 'distance': None, 'temperature' : None }

                    cur_node =  self.xml_parser.get_first_child(ex_node, 'ns3:hr')
                    if cur_node:
                        ex = mod_utils.to_number(self.xml_parser.get_node_data(cur_node))
                        extensions['hr'] = int(ex)

                    cur_node =  self.xml_parser.get_first_child(ex_node, 'ns3:cad')
                    if cur_node:
                        ex = mod_utils.to_number(self.xml_parser.get_node_data(cur_node))
                        extensions['cad'] = int(ex)
                        extensions['cadence'] = int(ex)

                    cur_node =  self.xml_parser.get_first_child(ex_node, 'ns3:speed')
                    if cur_node:
                        ex = mod_utils.to_number(self.xml_parser.get_node_data(cur_node))
                        extensions['speed'] = int(ex)

                    cur_node =  self.xml_parser.get_first_child(ex_node, 'ns3:distance')
                    if cur_node:
                        ex = mod_utils.to_number(self.xml_parser.get_node_data(cur_node))
                        extensions['distance'] = int(ex)

                    cur_node =  self.xml_parser.get_first_child(ex_node, 'ns3:atemp')
                    if cur_node:
                        ex = mod_utils.to_number(self.xml_parser.get_node_data(cur_node))
                        extensions['temperature'] = int(ex)
                        extensions['atemp'] = int(ex)
...
class TCXParser:
    ...
    def __parse_dom(self):
        node = self.xml_parser.get_first_child(name='TrainingCenterDatabase')
        if node is None:
            raise mod_gpx.GPXException('Document must have a `TrainingCenterDatabase` root node.')
        if self.xml_parser.get_node_attribute(node, "creator"):
            self.gpx.creator = self.xml_parser.get_node_attribute(node, "creator")
        ...
        for node in self.xml_parser.get_children(node):
            ...
            if node_name == 'time':
                ...
                self.gpx.routes.append(self._parse_route(node))
            elif node_name == 'Activities':

                # get the activity, process it.

                activity_node = self.xml_parser.get_first_child(node, 'Activity')
                #activity = self.xml_parser.get_node_data(name_node)

                self.gpx.tracks.append(self.__parse_track(activity_node))
            else:
                #print 'unknown %s' % node
                pass
    ...
    def __parse_track(self, node):
        ...
        name_node = self.xml_parser.get_first_child(node, 'Id')
        name = self.xml_parser.get_node_data(name_node)

        #type_node = self.xml_parser.get_first_child(node, 'type')
        #type = self.xml_parser.get_node_data(type_node)

        type = self.xml_parser.get_node_attribute(node, 'Sport')


        number = 0
        track = mod_gpx.GPXTrack(name, type, 0)
        track.type = type

        child_nodes = self.xml_parser.get_children(node)
        for child_node in child_nodes:
            if self.xml_parser.get_node_name(child_node) == 'Lap':
    ...
    def __parse_track_segment(self, node):
        ...
        trackitems = self.xml_parser.get_children(node)
        for cc in trackitems:
           if self.xml_parser.get_node_name(cc) == 'Track':

                child_nodes = self.xml_parser.get_children(cc)
                n = 0
                for child_node in child_nodes:
                    if self.xml_parser.get_node_name(child_node) == 'Trackpoint':
                        track_point = self.__parse_track_point(child_node)
                        track_segment.points.append(track_point)
                        n += 1
    ...
    def __parse_track_point(self, node):
        ...
        position_node = self.xml_parser.get_first_child(node, 'Position')

        latitude = self.xml_parser.get_node_attribute(position_node, 'LatitudeDegrees')
        if latitude:
            latitude = mod_utils.to_number(latitude)

        longitude = self.xml_parser.get_node_attribute(position_node, 'LongitudeDegrees')
        if longitude:
            longitude = mod_utils.to_number(longitude)
        ...
        elevation_node = self.xml_parser.get_first_child(node, 'AltitudeMeters')
        elevation = mod_utils.to_number(self.xml_parser.get_node_data(elevation_node),
                                        default=None, nan_value=None)
        ...
        if extensions_node:
            ...
            if ex_node:
                ...
                # get hr, cad, speed data
                #
                extensions = { 'hr': None, 'cad': None, 'speed': None, 'power': 0, 'left_pedal_smoothness': 0, 'left_torque_effectiveness': 0, 'distance': 0 }
